...
| `data.set_membership.member_unique` | Set membership chart audits | `PASS` when member identifiers are non-null, non-empty, and unique; `FAIL` for nulls or duplicates. | Prevents duplicate rows from silently inflating set-region counts. | Deduplicate to one row per universe member and fill missing identifiers. |
...
| `data.process_tree.columns` | Process-tree chart audits | `PASS` when declared node, parent, label, and optional branch columns exist; `FAIL` otherwise. | Verifies the structural fields needed to reconstruct the tree are present. | Add the missing columns or update the chart contract. |
| `data.process_tree.node_unique` | Process-tree chart audits | `PASS` when node IDs are non-null, non-empty, and unique; `FAIL` otherwise. | Prevents ambiguous or duplicated process nodes. | Give every node one unique non-null identifier. |
| `data.process_tree.labels` | Process-tree chart audits | `PASS` when every node has a non-empty label; `FAIL` otherwise. | Ensures each rendered process step can be interpreted. | Add a readable label for every node. |
| `data.process_tree.root` | Process-tree chart audits | `PASS` with exactly one root; `FAIL` with zero or multiple roots. | Enforces the rooted-tree contract. | Leave the parent field empty for exactly one entry node. |
| `data.process_tree.parent_reference` | Process-tree chart audits | `PASS` when every non-root parent ID exists; `FAIL` otherwise. | Prevents dangling process edges. | Point every non-root node to an existing node ID. |
| `data.process_tree.acyclic` | Process-tree chart audits | `PASS` when the parent graph is acyclic and rooted; `FAIL` when a cycle/disconnected structure is detected. | Keeps this intent a deterministic tree rather than a general graph. | Remove circular parent relationships or use a graph tool for genuinely cyclic workflows. |
| `visual.process_tree.structure` | First-party process-tree chart audits | `PASS` when rooted topology, edge direction, and optional branch labels are rendered by the first-party layout. | Makes the structural meaning inspectable without implying quantitative meaning from geometry. | Use `Chart.process_tree()` for the audited top-down layout. |
...
| `scale.bar.explicit_domain_zero` | Spec audits for bar marks | `FAIL` when an explicit numeric quantitative bar `domain`, `domainMin`, or `domainMax` excludes zero; otherwise no finding. | Ensures an explicit numeric bar domain cannot cut the zero baseline while avoiding duplicate findings for `scale.zero=false`. | Include zero in the quantitative bar domain or remove the explicit domain override. |
...
- Process-tree spec audits do not reconstruct rooted topology from arbitrary layered Vega-Lite output; use `Chart.process_tree().audit()` before rendering.
