...
| `data.process_tree.node_unique` | Process-tree chart audits | `PASS` when node IDs are non-null and unique; `FAIL` otherwise. | Prevents ambiguous or duplicated process nodes. | Give every node one unique non-null identifier. |
