...
| `data.distribution.sample_size` | Distribution chart audits | `PASS` with at least 20 non-null numeric metric observations; `WARN` for 5-19; `FAIL` below 5. | Checks whether the usable sample is large enough to summarize shape or spread. | Collect more valid observations before summarizing the distribution. |
| `data.distribution.group_sample_size` | Distribution chart audits with categories | `PASS` when each non-null group has at least 10 valid metric rows; `WARN` when any group is smaller. | Checks whether grouped distributions have enough usable rows per category. | Aggregate small groups or collect more data for each group. |
